...
namespace CCS.SspNet.Utilities
{
    internal static class CrcUtilities
    {
        ...
        internal static (byte LSB, byte MSB) CalculatePacketCrc(byte[] crcData)
        {
            var crc = CRC16_INIT;
            for (int i = 0; i < crcData.Length; ++i)
            {
                crc = (ushort)((crc << 8) ^ CRC16_LOOKUP[((crc >> 8) ^ (0xff & crcData[i]))]);
            }
            return ((byte)(crc & (ushort)0x00FF), (byte)(crc >> 8));
        }

#pragma warning disable IDE0051 // Remove unused private members
        private const ushort CRC16_POLY = 0x8005; // POLY used to generate CRC16_LOOKUP below.  Kept for reference.
#pragma warning restore IDE0051 // Remove unused private members
        private const ushort CRC16_INIT = 0xFFFF;
        private static readonly ushort[] CRC16_LOOKUP = { 
